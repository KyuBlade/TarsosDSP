...
public class PipeDecoder {
    ...
    private final String pipeProcess;
    ...
    public PipeDecoder() {
        ...
        if (isAvailable("ffmpeg")) {
            LOG.info(
                    "found ffmpeg on the path (" + path + "). Will use ffmpeg for decoding media files.");
            pipeProcess = "ffmpeg";
        } else if (isAvailable("avconv")) {
            LOG.info(
                    "found avconv on your path(" + path + "). Will use avconv for decoding media files.");
            pipeProcess = "avconv";
        } else {
            if (isAndroid()) {
                ...
                if (f.exists() && f.length() > 1000000 && f.canExecute()) {
                    ...
                    LOG.severe(
                            "Tried to unpack a statically compiled ffmpeg binary for your architecture to: " + f
                                    .getAbsolutePath());
                }
            } else {
                LOG.warning(
                        "Dit not find ffmpeg or avconv on your path(" + path + "), will try to download it automatically.");
                ...
                if (decoderBinaryAbsolutePath == null) {
                    LOG.severe(
                            "Could not download an ffmpeg binary automatically for your system.");
                }
            }
            if (decoderBinaryAbsolutePath == null) {
                throw new Error(
                ...
                pipeProcess = decoderBinaryAbsolutePath;
            }
        }
    }
    ...
    public PipeDecoder(String pipeEnvironment, String pipeArgument, String pipeProcess,
                       String pipeLogFile, int pipeBuffer) {
    ...
    public InputStream getDecodedStream(final String resource, final int targetSampleRate) {
        if (pipeProcess == null) {
            LOG.warning("Can't decode stream, no decoder found");
            return null;
        }

        try {
            ProcessBuilder pb;
            pb = new ProcessBuilder(pipeEnvironment, pipeArgument, pipeProcess,
                                    "-i", resource,
                                    "-vn",
